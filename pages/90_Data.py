...
def _is_grand_total(loc_val):
    s_val = _clean_loc(loc_val)
    if s_val is None:
        return False
    s_low = s_val.strip().lower()
    return s_low in {"grand total", "overall total", "company total", "total", "all"}


def _is_division_total(loc_val):
    s_val = _clean_loc(loc_val)
    if s_val is None:
        return False
    s_low = s_val.strip().lower()
...
def _build_landing_vs_ly_df(workbook_path_obj):
    ...
    if totals_df.shape[0] == 0:
        df_fallback = df_val.copy()
        df_fallback["Location"] = df_fallback[division_col].apply(_base_division_name)
        df_fallback["Location"] = df_fallback["Location"].apply(_clean_loc)

        df_fallback = df_fallback[df_fallback["Location"].notna()].copy()
        df_fallback = df_fallback[~df_fallback["Location"].astype(str).str.strip().str.lower().isin(EXCLUDE_DIVISIONS)].copy()

        want_set = {"digital", "screen print", "screenprint", "grand total"}
        df_fallback["__want"] = df_fallback["Location"].astype(str).str.strip().str.lower().isin(want_set)
        totals_df = df_fallback[df_fallback["__want"]].copy()
        totals_df = totals_df.drop(columns=["__want"])
...
def _build_trend_weekly_df(workbook_path_obj):
    ...
    date_col = _find_col(df_val, ["Week", "Week End", "Week Ending", "Date"])
